...
class _MainShellState extends State<MainShell> {
  ...
  Future<void> _toggleFavorite(String route) async {
    setState(() {
      _favorites.contains(route) ? _favorites.remove(route) : _favorites.add(route);
    });
    await _savePrefs();
  }
  ...
  Future<void> _ensureBillingState() async {
    ...
    try {
      ...
      final sent = (data['billingReminderMonth'] ?? '').toString() == reminderKey;
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (_, setModal) => AlertDialog(
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: ['purple', 'burgundy', 'red', 'navy', 'emerald', 'graphite'].map((value) => ChoiceChip(
                    label: Text(value[0].toUpperCase() + value.substring(1)),
          ...
          actions: [
            ...
            FilledButton(onPressed: () => Navigator.pop(dialogContext, [placement, background, button]), child: const Text('Apply')),
          ],
        ),
      ),
    );
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  OutlinedButton.icon(
                    onPressed: () { Navigator.pop(dialogContext); context.go(AppRouter.pricing); },
                    icon: const Icon(Icons.workspace_premium_outlined, size: 17),
                    label: const Text('Upgrade Package'),
                  ),
                  FilledButton.icon(
                    onPressed: () { Navigator.pop(dialogContext); context.go('${AppRouter.payment}/${plan == 'trial' ? 'monthly' : plan}'); },
  ...
  Future<void> _showNotificationHistory(UserModel user) async {
    await showDialog<void>(context: context, builder: (_) => _NotificationHistoryDialog(user: user));
  }
}
      ...
      child: Row(children: [
        ...
        SizedBox(
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                Flexible(child: Text(restaurant, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: _MainShellState.ink))),
                const SizedBox(width: 5),
                const Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: _MainShellState.muted),
              ]),
              Text('${user.branchName} • $title', overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10.5, color: _MainShellState.muted)),
            ],
          ),
        ),
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: const Row(children: [
                Icon(Icons.search_rounded, size: 20, color: _MainShellState.muted),
                SizedBox(width: 9),
                Expanded(child: Text('Search anything…', style: TextStyle(fontSize: 12, color: _MainShellState.muted))),
                _KeyHint('Ctrl /'),
              ]),
            ),
          ),
        ...
        OutlinedButton.icon(
          ...
          label: Text('Add Widget', style: TextStyle(color: accent, fontWeight: FontWeight.w800)),
          style: OutlinedButton.styleFrom(side: BorderSide(color: accent.withValues(alpha: .45)), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 14)),
        ),
        const SizedBox(width: 10),
        PopupMenuButton<bool>(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(color: online ? const Color(0xFFECFDF3) : _MainShellState.soft, borderRadius: BorderRadius.circular(22)),
            child: Row(children: [
              Icon(Icons.circle, size: 7, color: online ? const Color(0xFF12B76A) : _MainShellState.muted),
              const SizedBox(width: 6),
              Text(online ? 'Online' : 'Offline', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: online ? const Color(0xFF027A48) : _MainShellState.muted)),
              const Icon(Icons.arrow_drop_down_rounded, size: 16),
            ]),
          ),
        ),
        const SizedBox(width: 6),
        IconButton(tooltip: 'Packages, fees & usage', onPressed: onBilling, icon: Icon(Icons.account_balance_wallet_outlined, color: accent)),
        _NotificationButton(user: user, onTap: onNotifications),
        IconButton(tooltip: 'Interface', onPressed: onInterfaceSettings, icon: const Icon(Icons.tune_rounded)),
        ...
        PopupMenuButton<String>(
          ...
          itemBuilder: (_) => const [
            PopupMenuItem(value: 'profile', child: ListTile(leading: Icon(Icons.person_outline), title: Text('My Profile'))),
          ...
          child: Row(children: [
            ...
            ConstrainedBox(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(user.name.isEmpty ? 'User' : user.name, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w900)),
...
class _RestaurantLogo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (url.isEmpty) {
      return Container(width: size, height: size, decoration: BoxDecoration(color: const Color(0xFFF2F4F7), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.storefront_rounded, color: _MainShellState.navy));
    }
    return ClipRRect(
      borderRadius: BorderRadius.circular(12),
      child: Image.network(url, width: size, height: size, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(width: size, height: size, color: const Color(0xFFF2F4F7), child: const Icon(Icons.storefront_rounded))),
    );
  }
}
...
class _NotificationButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final query = FirebaseFirestore.instance.collection('vendors').doc(user.id).collection('notifications').orderBy('createdAt', descending: true).limit(50);
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: query.snapshots(),
      builder: (_, snapshot) {
        final count = (snapshot.data?.docs ?? const <QueryDocumentSnapshot<Map<String, dynamic>>>[]).where((doc) {
          final d = doc.data();
          final cleared = List<String>.from(d['clearedBy'] ?? const <String>[]).contains(user.authUid);
          final read = List<String>.from(d['readBy'] ?? const <String>[]).contains(user.authUid);
          return !cleared && !read && _visibleFor(user, d);
        }).length;
        return Stack(clipBehavior: Clip.none, children: [
          IconButton(tooltip: 'Notifications', onPressed: onTap, icon: const Icon(Icons.notifications_none_rounded)),
          if (count > 0)
            Positioned(right: 5, top: 4, child: Container(minWidth: 17, height: 17, padding: const EdgeInsets.symmetric(horizontal: 4), alignment: Alignment.center, decoration: BoxDecoration(color: const Color(0xFFD92D20), borderRadius: BorderRadius.circular(9)), child: Text(count > 99 ? '99+' : '$count', style: const TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.w900)))),
        ]);
      },
    );
  }
...
class _KeyHint extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(6), border: Border.all(color: _MainShellState.line)), child: Text(value, style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: _MainShellState.muted)));
}
...
class _BrandRail extends StatelessWidget {
  ...
  const _BrandRail({required this.user, required this.items, required this.currentRoute, required this.shortcutFor});
  @override
  Widget build(BuildContext context) => Container(
        ...
        child: Column(children: [
          ...
          Padding(padding: const EdgeInsets.symmetric(horizontal: 9), child: _RestaurantLogo(user: user, size: 54)),
          ...
          Expanded(
            child: ListView(
              ...
              children: items.map((item) {
                final selected = currentRoute == item.route || (item.route != '/' && currentRoute.startsWith(item.route));
                return Tooltip(
                  ...
                  child: InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: selected ? _MainShellState.purple : Colors.transparent, borderRadius: BorderRadius.circular(10)),
                      child: Stack(children: [
                        Center(child: Icon(item.icon, color: selected ? Colors.white : const Color(0xFFD0D5DD), size: 21)),
                        Positioned(right: 4, bottom: 4, child: Text(shortcutFor(item.label), style: TextStyle(fontSize: 7.5, fontWeight: FontWeight.w900, color: selected ? Colors.white : const Color(0xFF98A2B3)))),
                      ]),
                    ),
                  ),
                );
          ...
          Padding(
            ...
            child: Column(children: [
              ...
              Text('Powered by', style: TextStyle(fontSize: 7.5, color: Colors.white.withValues(alpha: .55))),
              const Text('TYCOON', style: TextStyle(fontSize: 8, color: Colors.white, fontWeight: FontWeight.w900, letterSpacing: .7)),
            ]),
          ),
        ]),
      );
...
class _SmartNavBar extends StatefulWidget {
  ...
  const _SmartNavBar({required this.items, required this.currentRoute, required this.accent, required this.background, required this.textColor, required this.favorites, required this.shortcutFor, required this.onFavorite});
  @override
...
class _SmartNavBarState extends State<_SmartNavBar> {
  ...
  Widget build(BuildContext context) {
    final sorted = [...widget.items]
      ..sort((a, b) => (widget.favorites.contains(b.route) ? 1 : 0).compareTo(widget.favorites.contains(a.route) ? 1 : 0));
    return LayoutBuilder(builder: (_, constraints) => MouseRegion(
        ...
        child: ScrollConfiguration(
          ...
          child: ListView(
            ...
            children: sorted.map((item) {
              final active = widget.currentRoute == item.route || (item.route != '/' && widget.currentRoute.startsWith(item.route));
              return Container(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: active ? widget.accent.withValues(alpha: .09) : widget.background,
                      borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: active ? widget.accent.withValues(alpha: .65) : widget.textColor.withValues(alpha: .12)),
                    ),
                    child: Row(children: [
                      Icon(item.icon, size: 17, color: active ? widget.accent : widget.textColor),
                      const SizedBox(width: 7),
                      Text(item.label, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 11.5, color: active ? widget.accent : widget.textColor)),
                      const SizedBox(width: 7),
                      _KeyHint(widget.shortcutFor(item.label)),
                      if (widget.favorites.contains(item.route)) ...[const SizedBox(width: 5), const Icon(Icons.star_rounded, size: 14, color: Color(0xFFF79009))],
                    ]),
                  ),
                ),
              );
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: ListView(children: items.map((item) {
      ...
      return ListTile(
        ...
        title: Text(item.label, style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w800, color: active ? accent : textColor)),
...
class _FeatureWorkspace extends StatefulWidget {
  ...
  final Widget child;
  const _FeatureWorkspace({required this.title, required this.accent, required this.favorite, required this.onFavorite, required this.child});
  @override
...
class _FeatureWorkspaceState extends State<_FeatureWorkspace> {
  ...
  Widget build(BuildContext context) {
    if (minimized) {
      return Align(alignment: Alignment.topLeft, child: Padding(padding: const EdgeInsets.all(18), child: OutlinedButton.icon(onPressed: () => setState(() => minimized = false), icon: const Icon(Icons.open_in_full_rounded), label: Text('Restore ${widget.title}'))));
    }
    return Container(
      color: const Color(0xFFF9FAFB),
      padding: EdgeInsets.all(maximized ? 0 : 14),
      child: Container(
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(maximized ? 0 : 14), border: maximized ? null : Border.all(color: _MainShellState.line), boxShadow: maximized ? null : const [BoxShadow(color: Color(0x0A101828), blurRadius: 16, offset: Offset(0, 5))]),
        child: Column(children: [
          Container(
            height: 46,
            ...
            child: Row(children: [
              Text(widget.title, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
              const Spacer(),
              IconButton(tooltip: widget.favorite ? 'Remove favorite' : 'Favorite', onPressed: widget.onFavorite, icon: Icon(widget.favorite ? Icons.star_rounded : Icons.star_border_rounded, size: 18, color: widget.favorite ? const Color(0xFFF79009) : _MainShellState.muted)),
              IconButton(tooltip: 'Minimize', onPressed: () => setState(() => minimized = true), icon: const Icon(Icons.remove_rounded, size: 18)),
              IconButton(tooltip: maximized ? 'Restore size' : 'Maximize', onPressed: () => setState(() => maximized = !maximized), icon: Icon(maximized ? Icons.close_fullscreen_rounded : Icons.open_in_full_rounded, size: 17)),
              IconButton(tooltip: 'Dashboard', onPressed: () => context.go(AppRouter.dashboard), icon: const Icon(Icons.close_rounded, size: 18)),
            ]),
          ),
...
class _UnreadNotificationRail extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final query = FirebaseFirestore.instance.collection('vendors').doc(user.id).collection('notifications').orderBy('createdAt', descending: true).limit(20);
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: query.snapshots(),
      builder: (_, snapshot) {
        ...
        return SizedBox(width: 355, child: Column(mainAxisSize: MainAxisSize.min, children: docs.map((doc) => Padding(padding: const EdgeInsets.only(bottom: 8), child: _NotificationToast(user: user, doc: doc))).toList()));
      },
    );
  }
}
...
class _NotificationToast extends StatelessWidget {
  ...
  Future<void> _read() => NotificationService().markRead(restaurantId: user.id, notificationId: doc.id, authUid: user.authUid);
  @override
  Widget build(BuildContext context) {
    final data = doc.data();
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFFFF4E5), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFF79009)), boxShadow: const [BoxShadow(color: Color(0x16000000), blurRadius: 10, offset: Offset(0, 3))]),
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Icon(Icons.notifications_active_outlined, color: Color(0xFFF79009), size: 20),
            ...
            IconButton(tooltip: 'Mark read & hide', onPressed: _read, icon: const Icon(Icons.done_rounded, color: Color(0xFFF79009), size: 18)),
            IconButton(tooltip: 'Dismiss', onPressed: () => NotificationService().dismiss(restaurantId: user.id, notificationId: doc.id, authUid: user.authUid), icon: const Icon(Icons.close_rounded, size: 17)),
          ]),
        ),
      ),
    );
...
class _NotificationHistoryDialogState extends State<_NotificationHistoryDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final query = FirebaseFirestore.instance.collection('vendors').doc(widget.user.id).collection('notifications').orderBy('createdAt', descending: true).limit(250);
    return Dialog(
      backgroundColor: Colors.white,
      ...
      child: SizedBox(
        ...
        child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
          stream: query.snapshots(),
          builder: (_, snapshot) {
            final all = (snapshot.data?.docs ?? const <QueryDocumentSnapshot<Map<String, dynamic>>>[]).where((doc) {
              final data = doc.data();
            ...
            return Column(children: [
              Padding(
                ...
                child: Row(children: [
                  ...
                  DropdownButton<String>(value: filter, underline: const SizedBox.shrink(), items: const [
                  ...
                  PopupMenuButton<String>(tooltip: 'Clear history view', onSelected: _clear, itemBuilder: (_) => const [
                ...
                child: docs.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          final read = List<String>.from(data['readBy'] ?? const <String>[]).contains(widget.user.authUid);
                          return InkWell(
                            onTap: () => NotificationService().markRead(restaurantId: widget.user.id, notificationId: doc.id, authUid: widget.user.authUid),
                            borderRadius: BorderRadius.circular(11),
                            child: Container(
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(color: read ? const Color(0xFFF8FAFC) : const Color(0xFFFFF4E5), borderRadius: BorderRadius.circular(11), border: Border.all(color: read ? _MainShellState.line : const Color(0xFFF79009))),
                              child: Row(children: [
                                Icon(read ? Icons.done_all_rounded : Icons.done_rounded, color: read ? const Color(0xFF2E90FA) : const Color(0xFFF79009), size: 19),
                                const SizedBox(width: 10),
                                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Text((data['title'] ?? 'Notification').toString(), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 12)),
                                  const SizedBox(height: 2),
                                ...
                                Text(read ? 'Read' : 'Unread', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w800, color: read ? const Color(0xFF2E90FA) : const Color(0xFFF79009))),
                              ]),
                            ),
                          );
                        },
  ...
  Future<void> _clear(String mode) async {
    ...
    if (mode == 'today') {
      ...
      final chosen = await showDatePicker(context: context, firstDate: DateTime(now.year - 5), lastDate: now, initialDate: now);
...
class _NavigationSearch extends SearchDelegate<NavigationItem?> {
  ...
  Widget _results(BuildContext context) {
    ...
    final result = items.where((e) => q.isEmpty || e.label.toLowerCase().contains(q)).toList();
    return ListView(children: result.map((item) => ListTile(leading: Icon(item.icon), title: Text(item.label), onTap: () => close(context, item))).toList());
  }
}
