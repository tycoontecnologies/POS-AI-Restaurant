...
class CartItem {
  ...
  CartItem({required this.product, this.variant, required this.quantity});
...
class CartProvider with ChangeNotifier {
  ...
  void clearCart() {
    _cartItems.clear();
    _selectedTable = null;
    // Reset table number when clearing cart
    notifyListeners();
  }
  ...
  void clearSelectedTable() {
    _selectedTable = null;
    // Reset table number when clearing selected table
    notifyListeners();
  }

  double get total => _cartItems.fold(0, (sum, item) => sum + item.totalPrice);
  ...
  Future<Sale> createSaleFromCart({
