...
class _PaymentScreenState extends State<PaymentScreen> {
    ...
    try {
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Opening secure payment…'),
  ...
  Future<void> _activateUsagePlan(String restaurantId) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Rs 1 per successful receipt package activated and receipts synchronized.',
          ),
  ...
  Future<void> _payUsage(double amount) async {
    if (amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('There is no outstanding usage balance.')),
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      if (opened && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Opening payment for Rs ${amount.toStringAsFixed(0)} transaction usage…',
            ),
  ...
  Future<void> _generateTycoonInvoice({
    ...
    try {
      await UsageBillingReconciliationService().reconcile(restaurantId);
      final usage = await vendorRef
          .collection('billingUsage')
          .where('status', isEqualTo: 'billable')
          .get();
      final now = DateTime.now();
      final invoiceNo =
          'TYC-${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}-${restaurantId.length > 6 ? restaurantId.substring(0, 6).toUpperCase() : restaurantId.toUpperCase()}';
        ...
        pw.MultiPage(
          ...
          build: (_) => [
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  pw.Text(
                    restaurantName.isEmpty ? restaurantId : restaurantName,
                  ),
            ...
            pw.Align(
              ...
              child: pw.Text(
                'TOTAL DUE: Rs ${due.toStringAsFixed(0)}',
                style: pw.TextStyle(
                  fontSize: 16,
                  fontWeight: pw.FontWeight.bold,
                ),
            ...
            pw.Text(
              'This invoice contains one charge for each unique successful receipt. Reprints do not create additional charges.',
              style: const pw.TextStyle(fontSize: 9, color: PdfColors.grey700),
            ),
            if (usage.docs.isNotEmpty) ...[
              pw.SizedBox(height: 24),
              pw.Text(
                'Billable receipt ledger',
                style: pw.TextStyle(fontWeight: pw.FontWeight.bold),
              ),
              pw.SizedBox(height: 8),
                ...
                return pw.Padding(
                  ...
                  child: pw.Row(
                    ...
                    children: [
                      pw.Expanded(
                        child: pw.Text(
                          (a['receiptId'] ?? a['saleId'] ?? d.id).toString(),
      ...
      if (mounted)
        setState(() => _errorMessage = 'Could not generate invoice: $e');
    } finally {
      if (mounted) setState(() => _isProcessing = false);
    }
  }
            ...
            child: _panel(
              children: [
                ...
                SizedBox(
                  ...
                  height: 48,
                  child: FilledButton(
                    onPressed: _isProcessing ? null : _processStandard,
                    style: _buttonStyle(),
                    child: _buttonChild('Proceed to payment'),
                  ),
                ),
    ...
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          final active =
              (data['billingPlanId'] ?? '').toString() == 'perTransaction' &&
              (data['hasActiveSubscription'] ?? false) == true;
          if (active && !_reconciled && !_reconciling) {
            WidgetsBinding.instance.addPostFrameCallback(
              (_) => _reconcileUsage(user.id),
            );
          }
          final lifetimeReceipts = (data['successfulReceiptCount'] is num)
          ...
          final rate = (data['transactionRate'] is num)
              ...
              : 1.0;
          final rawDue = (data['transactionUsageAmount'] is num)
              ? (data['transactionUsageAmount'] as num).toDouble()
              : unbilled * rate;
          final due = rawDue < 0 ? 0.0 : rawDue;
          final paid = (data['transactionPaidTotal'] is num)
              ? (data['transactionPaidTotal'] as num).toDouble()
              : 0.0;
          final paidReceipts = (data['transactionPaidReceiptTotal'] is num)
              ? (data['transactionPaidReceiptTotal'] as num).toInt()
              : 0;
          final dueRaw = data['nextPaymentDueAt'];
          final dueDate = dueRaw is Timestamp ? dueRaw.toDate() : null;
          final status =
              (data['billingStatus'] ?? (active ? 'active' : 'not active'))
                  .toString();
                ...
                child: _panel(
                  children: [
                        ...
                        const Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 5),
                              Text(
                                'Rs 1 is charged once for each unique successfully completed receipt. Reprints are not charged again.',
                                style: TextStyle(
                                  color: muted,
                                  fontSize: 12.5,
                                  height: 1.45,
                    ...
                    if (_reconciling) ...[
                      ...
                      const Text(
                        'Synchronizing completed receipts with the Tycoon billing ledger…',
                        ...
                        children: [
                          Expanded(
                            child: OutlinedButton.icon(
                              onPressed: _isProcessing || _reconciling
                                  ? null
                                  : () => _reconcileUsage(user.id),
                    ...
                    SizedBox(
                      ...
                      child: FilledButton(
                        onPressed: _isProcessing
                            ? null
                            : active
                            ? (due > 0 ? () => _payUsage(due) : null)
                            : () => _activateUsagePlan(user.id),
                        style: _buttonStyle(),
                        child: _buttonChild(
                          active
                              ? (due > 0
                                    ? 'Pay Rs ${due.toStringAsFixed(0)} usage fee'
                                    : 'No amount due')
                              : 'Activate Rs 1 / receipt package',
                        ),
                      ),
                    ),
                    if (active && due <= 0) ...[
                      ...
                      const Center(
                        child: Text(
                          'Your current transaction usage is fully settled.',
                          style: TextStyle(
      ...
      child: Column(
        ...
        children: [
          const Padding(
            ...
            child: Row(
              children: [
                Icon(Icons.list_alt_rounded, size: 18, color: burgundy),
                SizedBox(width: 8),
                Text(
                  'Recent receipt billing ledger',
            ...
            builder: (_, snap) {
              ...
              if (docs.isEmpty)
                return const Padding(
                  ...
                  child: Text(
                    'No successful receipts have been metered yet. Use Sync Receipts to reconcile completed sales.',
              ...
              return Column(
                children: docs.map((d) {
                  ...
                  final rawAmount =
                      a['amount'] ?? a['billableAmount'] ?? a['rate'] ?? 1;
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          flex: 2,
                          child: Text(
                            'Receipt ${a['receiptId'] ?? a['saleId'] ?? d.id}',
  ...
  Widget _error() => _errorMessage == null
      ...
      : Padding(
          ...
          child: Container(
            ...
            child: Text(
              _errorMessage!,
              style: const TextStyle(color: Color(0xFFBE123C), fontSize: 11.5),
            ),
          ),
        );
