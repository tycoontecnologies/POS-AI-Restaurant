...
class _StoreOutScreenState extends State<StoreOutScreen> {
  ...
  Future<void> _delete(StoreOut item) async {
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Text('Remove ${item.reason.isEmpty ? 'this store movement' : item.reason}?'),
        actions: [
  ...
  Widget build(BuildContext context) {
    ...
    final todayCount = items.where((e) => e.date.year == now.year && e.date.month == now.month && e.date.day == now.day).length;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                const Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Store Overview', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: _ink)),
                      SizedBox(height: 3),
                      Text('Outgoing stock, kitchen movement and internal usage', style: TextStyle(fontSize: 11, color: _muted)),
                    ],
                  ),
                ),
                FilledButton.icon(
            ...
            LayoutBuilder(
              builder: (_, c) {
                final cols = c.maxWidth >= 950 ? 4 : c.maxWidth >= 600 ? 2 : 1;
              ...
              child: LayoutBuilder(
                builder: (_, c) {
                  ...
                  final views = Wrap(
                    ...
                    runSpacing: 6,
                    children: [
                      _ViewButton(label: 'Cards', icon: Icons.grid_view_rounded, selected: _view == 'cards', onTap: () => setState(() => _view = 'cards')),
                      _ViewButton(label: 'Compact', icon: Icons.view_list_rounded, selected: _view == 'list', onTap: () => setState(() => _view = 'list')),
                      _ViewButton(label: 'Table', icon: Icons.table_rows_rounded, selected: _view == 'table', onTap: () => setState(() => _view = 'table')),
                    ],
                  );
                  if (compact) {
                    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [search, const SizedBox(height: 8), views]);
                  }
                  return Row(children: [Expanded(child: search), const SizedBox(width: 10), views]);
                },
              ),
            ),
            const SizedBox(height: 12),
            if (provider.error != null)
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(provider.error!, style: const TextStyle(color: _red))),
                  IconButton(onPressed: provider.clearError, icon: const Icon(Icons.close_rounded)),
                ]),
              ),
            Expanded(
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, style: const TextStyle(fontSize: 10, color: _muted, fontWeight: FontWeight.w700)),
            const SizedBox(height: 3),
            Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: _ink)),
          ])),
        ]),
      );
}
...
class _ViewButton extends StatelessWidget {
  ...
  const _ViewButton({required this.label, required this.icon, required this.selected, required this.onTap});
        ...
        builder: (_, c) {
          ...
          return SingleChildScrollView(
            ...
            child: Wrap(
              spacing: 10,
              runSpacing: 10,
              children: items.map((item) => SizedBox(width: width, child: _MovementCard(item: item, onEdit: onEdit, onDelete: onDelete))).toList(),
            ),
          );
        },
      );
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFFFFAEB), borderRadius: BorderRadius.circular(7)),
            child: Text(item.reason.isEmpty ? 'Movement' : item.reason, style: const TextStyle(fontSize: 9, color: _orange, fontWeight: FontWeight.w900)),
          ),
          const Spacer(),
          ...
          return Container(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(item.reason, style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w900)),
                Text(item.products.map((e) => '${e.product.name} (${e.quantity})').take(2).join(', '), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9, color: _muted)),
              ])),
              Expanded(child: _Meta(Icons.inventory_2_outlined, '${item.items} units')),
              ...
              child: DataTable(
                ...
                rows: items.map((item) => DataRow(cells: [
                  ...
                  DataCell(Row(children: [
                    IconButton(onPressed: () => onEdit(item), icon: const Icon(Icons.edit_outlined, color: _purple)),
                    IconButton(onPressed: () => onDelete(item), icon: const Icon(Icons.delete_outline_rounded, color: _red)),
                  ])),
                ])).toList(),
              ),
            ),
...
class _Meta extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Flexible(child: Text(text, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.5, color: _muted))),
      ]);
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 10),
          const Text('No store movements found', style: TextStyle(fontWeight: FontWeight.w800, color: _ink)),
          const SizedBox(height: 10),
