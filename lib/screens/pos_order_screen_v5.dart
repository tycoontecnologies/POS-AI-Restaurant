...
class _TableOrderScreenState extends State<TableOrderScreen> {
  ...
  void didChangeDependencies() {
    ...
    orders.loadTableOrder(widget.table.id).then((_) {
      if (!mounted) return;
      setState(() {
        _kotSent = orders.getOrderStatus(widget.table.id) == 'making';
      });
    });
  }
  ...
  Future<void> _printKot() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('KOT printed • Order in Making'),
  ...
  String _billId(TableOrderProvider orders) {
    final info = orders.getOrderInfo(widget.table.id);
    final created = info['createdAt'];
    return created == null
        ? 'bill_${widget.table.id}'
        : 'bill_${widget.table.id}_${created.hashCode}';
  }
  ...
  Future<void> _markServed() async {
    await context
        ...
        .setOrderStatus(widget.table.id, 'served');
    await context
        .read<TableProvider>()
        .updateTableStatus(widget.table.id, TableStatus.served);
    if (mounted) setState(() {});
  }
  ...
  Future<void> _checkout() async {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        'Receipt will print “Paid via: $method”.',
                        style: const TextStyle(
                          fontSize: 11,
    ...
    if (result != null) {
      await _completeSale(result.method, result.reference);
    }
  }
  ...
  Future<void> _completeSale(String method, String? reference) async {
    ...
    try {
      ...
      await context
          .read<TableProvider>()
          .updateTableStatus(widget.table.id, TableStatus.empty);
  ...
  Widget build(BuildContext context) {
    ...
    final status = currentTable?.status ?? widget.table.status;
    final orderStatus = orders.getOrderStatus(widget.table.id);
    final served = status == TableStatus.served || orderStatus == 'served';
        ...
        children: [
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Tables / Table ${table.tableNumber}',
                  style: const TextStyle(
                    fontSize: 10,
                  ...
                  if (!kotSent && !served)
                    ...
                    SizedBox(
                      ...
                      child: FilledButton.icon(
                        ...
                        label: Text(
                          'CHECKOUT • Rs ${total.toStringAsFixed(0)}',
                        ),
                      ),
                    ),
                  const SizedBox(height: 7),
